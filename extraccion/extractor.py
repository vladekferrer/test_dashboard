...
def transformar_partner(r):
    return {
        ...
        "industry_id": _normalizar_many2one(r.get("industry_id")),
        "extracted_at": datetime.now().isoformat(),
    }
...
def transformar_product(r):
    def limpiar(texto, largo=256):
        if not texto:
            return None
        # Eliminar caracteres que SQLite no digiere bien
    ...
    # Manejar categ_id de forma segura por si viene como lista o int
    cat_val = r.get("categ_id")
    categ_id_limpio = cat_val[0] if isinstance(cat_val, (list, tuple)) and cat_val else (cat_val if isinstance(cat_val, int) else None)

    return {
        "id": r["id"],
        "name": limpiar(r.get("name"), 256),
        "default_code": limpiar(r.get("default_code"), 64),
        "barcode": limpiar(r.get("barcode"), 64),
        "list_price": r.get("list_price", 0) or 0,
        "standard_price": r.get("standard_price", 0) or 0,
        "categ_id": categ_id_limpio,
        "type": r.get("type"),
...
def ejecutar_extraccion(modo: str = "inicial"):
    ...
    extracciones = [
        ...
        ("product.product",   CAMPOS_PRODUCT,          ['|', ["active", "=", True], ["active", "=", False]],
         "raw_product",       transformar_product),
        
        # NUEVO BLOQUE DE SECTORES / INDUSTRIA
        ("res.partner.industry", ["id", "name", "full_name"], [],
         "raw_partner_industry", lambda r: {
             "id": r["id"],
             "name": r.get("name"),
        ...
        ("res.partner",       CAMPOS_PARTNER,          ['|', ["company_id", "=", 2], ["company_id", "=", False]],
         "raw_partner",       transformar_partner),
        
        ("sale.order",        CAMPOS_SALE_ORDER,       
         [["company_id", "=", 2], ["date_order", ">=", fecha_corte], ["advisory", "not in", [50, 59]]],
         "raw_sale_order",    transformar_sale_order),
        
        ("sale.order.line",   CAMPOS_SALE_ORDER_LINE,  
         [["company_id", "=", 2], ["create_date", ">=", fecha_corte]],
         "raw_sale_order_line", transformar_sale_order_line),
        
        ("account.move",      CAMPOS_ACCOUNT_MOVE,
         [["company_id", "=", 2], ["move_type", "in", ["out_invoice", "out_refund"]], ["invoice_date", ">=", fecha_corte], ["advisory", "not in", [50, 59]]],
