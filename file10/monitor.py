    ...
    with engine.connect() as conn:
        ...
        cartera = conn.execute(text("""
            ...
            JOIN raw_sale_order so
                ON so.partner_id = am.partner_id
...
# ============================================================
# CUENTAS EN RIESGO (Ignora historial, busca dueño actual)
# ============================================================

def cuentas_en_riesgo(dias: int = 14) -> list:
    """
    Cuentas del top 50 sin pedido en los últimos N días, por vendedor actual.

    Incluye cartera pendiente por cliente y clasificación de tipo de inactividad:
      CARTERA   → tiene deuda pendiente, la inactividad es por bloqueo de cobro.
      DATOS     → >90 días sin pedido y sin cartera: posible cambio de razón social,
                  cierre o duplicado. Requiere depuración, no visita.
      COMERCIAL → inactividad genuina de ventas. Aquí sí aplica visitar y proponer.
    """
    engine = _engine()
    with engine.connect() as conn:
        rows = conn.execute(text("""
            WITH asesores_actuales AS (
                SELECT rps.partner_id, GROUP_CONCAT(DISTINCT ru.name) AS nombres_asesores
                FROM raw_partner_salesperson rps
                JOIN raw_user ru ON ru.id = rps.salesperson_id
                WHERE UPPER(ru.name) NOT LIKE '%OFICINA%' 
                  AND UPPER(ru.name) NOT LIKE '%SILVANA%'
                GROUP BY rps.partner_id
            ),
            cartera_cliente AS (
                SELECT
                    partner_id,
                    ROUND(SUM(amount_residual), 0)            AS cartera_pendiente,
                    ROUND(SUM(CASE
                        WHEN julianday('now') -
                             julianday(invoice_date_due) > 30
                        THEN amount_residual ELSE 0
                    END), 0)                                  AS cartera_vencida_30d,
                    MAX(CAST(julianday('now') -
                        julianday(invoice_date_due) AS INT))  AS max_dias_vencido
            ...
            SELECT
                rp.name                                              AS cliente,
                COALESCE(aa.nombres_asesores, 'Sin asignar')         AS vendedor,
                MAX(so.date_order)                                   AS ultimo_pedido,
                CAST(julianday('now') -
                     julianday(MAX(so.date_order)) AS INT)           AS dias_inactivo,
                ROUND(SUM(so.amount_untaxed) /
                      COUNT(DISTINCT strftime('%Y-%m', so.date_order)), 0) AS gmv_mensual_prom,
                COUNT(DISTINCT strftime('%Y-%m', so.date_order))       AS meses_activo,
                COALESCE(cc.cartera_pendiente, 0)     AS cartera_pendiente,
                COALESCE(cc.cartera_vencida_30d, 0)   AS cartera_vencida_30d,
                COALESCE(cc.max_dias_vencido, 0)      AS max_dias_vencido
            FROM raw_sale_order so
            JOIN raw_partner rp ON so.partner_id = rp.id
            LEFT JOIN asesores_actuales aa ON aa.partner_id = rp.id
            LEFT JOIN cartera_cliente cc   ON cc.partner_id = so.partner_id
            WHERE so.company_id = :cid
              AND so.state IN ('sale','done')
              AND so.partner_id != 3
            ...
            GROUP BY so.partner_id, aa.nombres_asesores
            HAVING dias_inactivo > :dias
            ORDER BY dias_inactivo DESC, gmv_mensual_prom DESC
        """), {"cid": COMPANY_ID, "dias": dias}).mappings().fetchall()

    # ── Clasificar tipo de inactividad ────────────────────────────
    # Determinista: el LLM recibe la clasificación hecha, no adivina.
    resultado = []
    for r in rows:
        d = dict(r)
        cp = d.get("cartera_pendiente", 0) or 0
        cv = d.get("cartera_vencida_30d", 0) or 0
        di = d.get("dias_inactivo", 0) or 0

        if cp > 0 and cv > 0:
            d["tipo_inactividad"] = "CARTERA"
            d["accion_correcta"] = (
                "Gestión de cobro: verificar estado de pago, esperar soporte, "
                "escalar si es necesario. NO proponer productos."
            )
        elif di > 90 and cp == 0:
            d["tipo_inactividad"] = "DATOS"
            d["accion_correcta"] = (
                "Verificar en Odoo si el cliente cambió de razón social, "
                "cerró o es un duplicado. Depurar, NO visitar."
            )
        else:
            d["tipo_inactividad"] = "COMERCIAL"
            nota_cartera = ""
            if cp > 0 and cv == 0:
                nota_cartera = (
                    f" OJO: tiene cartera pendiente de ${cp:,.0f} dentro de "
                    f"plazo — no está bloqueado, pero verificar antes de despachar."
                )
            d["accion_correcta"] = (
                "Inactividad genuina. Visitar con propuesta de categorías "
                "faltantes del portafolio." + nota_cartera
            )
        resultado.append(d)
        ...
        rows = conn.execute(text(f"""
            ...
            ventas_cat AS (
                SELECT
                    so.partner_id,
                    CASE
                        WHEN UPPER(COALESCE(rpt.name, rp.name, '')) LIKE '%WHISKY%' OR UPPER(COALESCE(rpt.name, rp.name, '')) LIKE '%WHISKEY%' THEN 'Whisky'
                        WHEN UPPER(COALESCE(rpt.name, rp.name, '')) LIKE '%VINO%' OR UPPER(COALESCE(rpt.name, rp.name, '')) LIKE '%WINE%' THEN 'Vinos'
                        WHEN UPPER(COALESCE(rpt.name, rp.name, '')) LIKE '%ESPUMANTE%' OR UPPER(COALESCE(rpt.name, rp.name, '')) LIKE '%CHAMPAGNE%' OR UPPER(COALESCE(rpt.name, rp.name, '')) LIKE '%PROSECCO%' THEN 'Espumantes'
                        WHEN UPPER(COALESCE(rpt.name, rp.name, '')) LIKE '%GIN%' OR UPPER(COALESCE(rpt.name, rp.name, '')) LIKE '%GINEBRA%' THEN 'Gin'
                        WHEN UPPER(COALESCE(rpt.name, rp.name, '')) LIKE '%RON%' OR UPPER(COALESCE(rpt.name, rp.name, '')) LIKE '%RUM%' THEN 'Ron'
                        ELSE NULL
                    END AS categoria
                FROM raw_sale_order so
                JOIN raw_sale_order_line sol ON sol.order_id = so.id
            ...
            LEFT JOIN asesores_actuales aa ON aa.partner_id = tc.partner_id
...
def codificacion_marcas(top_n: int = 20) -> list:
    """Para cada cuenta top, qué marcas estratégicas ha comprado en los últimos 3 meses."""
    engine = _engine()
    with engine.connect() as conn:
        filas = []
        ...
        top = conn.execute(text("""
            ...
            SELECT so.partner_id, rp.name AS cliente,
                   COALESCE(aa.nombres_asesores,'Sin asignar') AS vendedor
            FROM raw_sale_order so
            JOIN raw_partner rp ON so.partner_id = rp.id
            LEFT JOIN asesores_actuales aa ON aa.partner_id = rp.id
    ...
    for t in top:
        ...
        marcas_faltantes = [m for m in MARCAS_ESTRATEGICAS if m not in marcas_activas]
        resultado.append({
            "cliente":          t["cliente"],
            "vendedor":         t["vendedor"],
