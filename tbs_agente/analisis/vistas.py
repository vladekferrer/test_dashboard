...
cartera_hoy AS (
    SELECT
        ...
        COALESCE(SUM(CASE
            ...
            THEN am.amount_residual ELSE 0 END), 0)              AS cartera_vencida_30,
        COALESCE(AVG(CASE
            WHEN am.payment_state = 'paid'
            THEN julianday(am.invoice_date) - julianday(am.invoice_date_due)
            ELSE NULL END), 0)                                   AS dias_cobro_prom
...
margen AS (
    SELECT
        CASE
            WHEN SUM(COALESCE(rpt.standard_price,
                              rp.standard_price, 0)
                     * sol.product_uom_qty) > 0
            THEN 1.0 - (
                SUM(COALESCE(rpt.standard_price,
                             rp.standard_price, 0)
                    * sol.product_uom_qty) /
...
SELECT
    ...
    ROUND(mant.gmv_neto, 0)                                      AS gmv_neto_anterior,
    ROUND((ma.gmv_neto - mant.gmv_neto) /
          NULLIF(mant.gmv_neto, 0) * 100, 1)                    AS gmv_variacion_pct,
    ma.cuentas_activas                                           AS cuentas_activas,
    mant.cuentas_activas                                         AS cuentas_activas_anterior,
    ROUND(m.margen_bruto_pct * 100, 1)                          AS margen_bruto_pct,
    ...
    ROUND(ABS(c.dias_cobro_prom), 0)                            AS dias_cobro_promedio,
    ma.ticket_promedio                                           AS ticket_promedio
FROM mes_actual ma, mes_anterior mant, cartera_hoy c, margen m
"""
...
SELECT
    ...
    CASE
        WHEN SUM(costo) > 0
        THEN ROUND((SUM(revenue) - SUM(costo)) /
...
categorias_cliente AS (
    ...
    FROM raw_sale_order so
    JOIN raw_sale_order_line sol ON sol.order_id = so.id
    JOIN raw_product rp ON sol.product_id = rp.id
