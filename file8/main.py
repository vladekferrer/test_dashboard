...
# ---> ACÁ AGREGAMOS LA LÍNEA DEL AGENTE (Línea 15) <---
from agente.agente_routes import router as agente_router

app = FastAPI(title="TBS Dashboard API", version="2.0.0")

# ---> ACÁ ENCHUFAMOS EL AGENTE A LA APP (Línea 20) <---
...
def agente_dashboard():
    # Suponiendo que metiste el agente.html en la misma carpeta frontend
    return FileResponse(FRONTEND_DIR / "agente.html")
...
def regenerar_insight():
    try:
        from llm.openai_client import generar_insight_diario, guardar_insight_en_db
...
def chat(req: PreguntaRequest):
    ...
    try:
        from llm.openai_client import responder_pregunta
