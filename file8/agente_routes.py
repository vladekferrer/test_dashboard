...
def generar_analisis_estrategico():
    """Genera un análisis estratégico fresco."""
    try:
...
# ════════════════════════════════════════════════════════
# CUOTAS (leidas desde cuotas.xlsx)
# ════════════════════════════════════════════════════════

@router.get("/cuotas")
def get_cuotas():
    """
    Devuelve las cuotas actuales por vendedor + el desglose por proveedor
    (cuando aplica) + el estado de carga del Excel.

    Util para que el dashboard muestre las cuotas que estan en vigor
    y permita ver si el archivo se cargo bien o si esta usando defaults.
    """
    from agente.cuotas import (
        get_todos_vendedores, get_cuota, get_plan_proveedores,
        estado_carga, VENDEDOR_A_ZONA,
    )

    items = []
    for nombre in get_todos_vendedores():
        items.append({
            "vendedor":            nombre,
            "zona":                VENDEDOR_A_ZONA.get(nombre),
            "cuota":               get_cuota(nombre),
            "plan_por_proveedor":  get_plan_proveedores(nombre),
        })
    return {
...
@router.post("/cuotas/recargar")
def recargar_cuotas():
    """
    Fuerza la relectura de cuotas.xlsx ignorando el cache de mtime.
    Util si edito el archivo y quiero ver el cambio sin esperar a que
    el sistema lo detecte por timestamp.
    """
    from agente.cuotas import recargar
    return recargar()
