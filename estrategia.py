...
# Nombres TAL COMO los tiene Odoo en raw_product_category (en plural).
# Son los mismos 7 conceptos de siempre; "Espumante" en Odoo está partido
# en tres categorías: Espumosos, Champagnes y Prosecco.
# Si en Odoo se unifican o renombran categorías, basta con ajustar esta lista.
CATEGORIAS_PREMIUM = [
    "Whiskys", "Vinos", "Espumosos", "Champagnes", "Prosecco",
    "Ginebras", "Mezcales", "Rones", "Tequilas",
]
...
def obj_mix_premium() -> dict:
    ...
    La categoría de cada producto se toma de categ_id -> raw_product_category
    (la clasificación real de Odoo), con la misma lógica que la vista
    v_categorias. NO se adivina del nombre del producto.
    """
    engine = _engine()
    # CATEGORIAS_PREMIUM es una constante interna del módulo (sin input de
    # usuario), por eso es seguro interpolarla directamente en el SQL.
    premium_sql = ", ".join(f"'{c}'" for c in CATEGORIAS_PREMIUM)
    ...
    with engine.connect() as conn:
        rows = conn.execute(text(f"""
            WITH base AS (
                ...
                LEFT JOIN raw_product_template rpt ON rpt.id = rp.product_tmpl_id
                                                  AND rpt.company_id = :cid
                LEFT JOIN raw_product_category rc  ON rc.id = rpt.categ_id
                WHERE so.company_id = :cid
                  AND so.state IN ('sale','done')
                  AND so.date_order >= date('now', '-5 months')
            ),
            ...
            SELECT
                mes,
                ROUND(SUM(revenue), 0)                AS gmv_total,
                ROUND(SUM(CASE
                    WHEN categoria IN ({premium_sql})
                    THEN revenue ELSE 0
